...
def draw_tracking_overlay(
    ...
    if status == TrackingStatus.SEARCHING:
        cv2.putText(frame, "STATUS: SEARCHING", (10, 80),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2, cv2.LINE_AA)  # Green
        cv2.putText(frame, "Looking for drone...", (10, 110),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2, cv2.LINE_AA)  # Green
        return
    ...
    if rpm > 0:
        ...
        cv2.putText(
            frame, f"RPM ESTIMATE: {rpm:.0f}", 
            (10, 140), cv2.FONT_HERSHEY_SIMPLEX, 0.6, rpm_color, 2, cv2.LINE_AA
        )
    else:
        cv2.putText(
            frame, "CALCULATING RPM...:", (10, 140),
...
def main() -> None:
    ...
    rpm_tracker = FFT_RPMTracker(history_ms=250, sample_rate_hz=5000)
    # ---
